...
import com.myspringboot.utils.SerializeUtils;
...
public class ShrioRedisCache<K, V> implements Cache<K, V> {
	...
	public ShrioRedisCache(RedisTemplate redisTemplate) {
		this.redisTemplate = redisTemplate;
	}

	public ShrioRedisCache(RedisTemplate redisTemplate, String prefix) {
		this(redisTemplate);
		this.prefix = prefix;
	}
	...
	public V get(K key) throws CacheException {
		...
		return (V) redisTemplate.opsForValue().get(bkey);
	}
	...
	public Collection<V> values() {
		...
		for(K k: keys) {
			...
			values.add((V) redisTemplate.opsForValue().get(bkey));
		}
		return values;
	}
