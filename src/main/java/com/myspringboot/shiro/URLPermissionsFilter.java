...
import com.myspringboot.userinfo.UserService;
...
public class URLPermissionsFilter extends PermissionsAuthorizationFilter{
	...
	public boolean isAccessAllowed(ServletRequest request, ServletResponse response, Object mappedValue) throws IOException {
		...
		if(subject.getPrincipal() == null 
				...
				|| StringUtils.equals(curUrl, "/unauthor")|| StringUtils.equals(curUrl, "/ws/sockjs/info")) {
			return true;
		}
		List<String> urls = userService.findPermissionUrl(subject.getPrincipal().toString());
		String mainurl = curUrl;
		if(curUrl.substring(1).indexOf("/")>0){
			mainurl = curUrl.substring(0,curUrl.substring(1).indexOf("/")+1);
		}
		return urls.contains(mainurl);
	}
